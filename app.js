...
function removeUrlParameterByName(name) {
    ...
    try {
        window.history.replaceState(null, null, window.location.origin + location.pathname + (empty(new_url) ? '' : (new_url.length == 0 ? '' : "?" + new_url.substring(1))));
    } catch (e) {
        console.log(e)
    }
}
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post("//" + __api_domain + "/user/services/api", {
                    cmd: 'updateBillingData',
                    formData: JSON.stringify({
                        billing_info: result.value
                    }),
                    ...
                    if (!empty(response.data)) {
                        ...
                        if (result.value.country == 'IN' && !empty(result.value.gst_pin)) {
                            $("#billing_info_gstpin").html("<strong>GST PIN: </strong>" + result.value.gst_pin)
                            $("#billing_info_gstpin").show()
                            $("#billing_info_taxid").hide()
                        } else if (!empty(result.value.vat_id)) {
                            $("#billing_info_taxid").html("<strong>VAT/TAX ID: </strong>" + result.value.vat_id)
                            $("#billing_info_taxid").show()
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let data = {
                ...
                formData: JSON.stringify({
                    locale,
                    redirect_domain: locale + ".qrcodechimp.com"
                })
            }
